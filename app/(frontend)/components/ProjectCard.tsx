  ...
  return (
    <div className="group relative overflow-hidden bg-surface-container border border-black/10 dark:border-white/10 p-6 md:p-8 h-full min-h-[400px] flex flex-col justify-end w-full">
      ...
      <div className="absolute inset-0 z-0">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-white/90 via-white/40 to-transparent dark:from-black/90 dark:via-black/40 z-10"></div>
      </div>
      ...
      <div className="relative z-20">
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {project.tech.slice(0, 3).map((t) => (
            <span key={t} className="text-[10px] font-code-sm px-2 py-0.5 bg-black/5 dark:bg-white/5 text-on-surface-variant border border-black/10 dark:border-white/10 uppercase">
              {t}
            </span>
          ))}
        </div>
        ...
        <div className="flex gap-4">
          ...
          <Link 
            href={project.github} 
            target="_blank"
            className="border border-black/20 dark:border-white/20 bg-black/5 dark:bg-white/5 text-on-surface px-5 py-2.5 font-code-sm text-xs uppercase font-bold flex items-center gap-2 hover:bg-black/10 dark:hover:bg-white/10 active:scale-95 transition-all"
          >
            GitHub
            <Image src="/icons/github.svg" alt="GitHub" width={14} height={14} className="dark:invert" />
          </Link>
        </div>
      </div>
    </div>
